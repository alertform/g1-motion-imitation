...
**v11 训练中**（单段行走 `walk1_subject1`，800M 步，约 16 小时）。

历代最好结果 v9：MJX 下 16 个起点存活均值 **275.9 步**（零动作前馈基线 54.1，比值 **5.10**），
其中 2 个起点跑满 500 步上限。完整实验记录见 [docs/experiments.md](docs/experiments.md)。

尚未解决：
- **根漂移约 25cm** 且对权重不敏感，疑似当前奖励配比下的平衡点
- **动作僵硬 / 稳定性权衡**：v9 阻尼过高显僵硬，v10 放开后不稳，v11 正在验证分组阻尼
...
bash scripts/run-eval-mjx.sh --episodes 16 --max-steps 500
