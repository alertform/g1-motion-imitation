...
**v15 完成**，单段行走 `walk1_subject1`。MJX 下 16 个起点 × 1500 步评估：
...
| 策略/前馈 | 1.00 | **30.88** |
| 根漂移 | 14.69cm | **4.36cm** |
| 根速度误差 | 0.676 m/s | **0.192 m/s** |
| 关节误差 | 3.63° | 5.62° |

**15/16 个起点跑满 1500 步（30 秒不摔）**，最后一个 1343 步。
完整实验记录见 [docs/experiments.md](docs/experiments.md)。

近三版的关键改动：
- **v14** 笛卡尔身体跟踪奖励（9 body 相对 pelvis，替代关节角主项）
- **v15** RSI 起点过滤 —— 排除 `d.ncon < 2` 的单点支撑帧。定位靠对照实验：
  唯一以 -4.0σ 区分「崩」与「跑满」的指标是接触点数，不是姿态或速度

下一步方向：

- **多段数据** —— 68 段（43.8 万帧）尚未使用，参考 PHC 的单策略多动作方案
- **域随机化 + sim2real** —— PD 增益本身就在标准随机化清单里
- **失败恢复** —— 现在摔了直接终止，PHC 用 AMP 学爬起来
